 ...
 */
package tech.metacontext.ocnhfa.cf_composer.model.devices;

import java.util.LinkedList;
import java.util.List;
import java.util.stream.Collectors;
import tech.metacontext.ocnhfa.cf_composer.enums.*;
import tech.metacontext.ocnhfa.cf_composer.model.y.PitchMove;
...
//    private LinkedList<PitchNode> melody;

    public CantusFirmus(EcclesiasticalMode ecclesiastical_mode) {

        this.ecclesiastical_mode = ecclesiastical_mode;
        var path = ecclesiastical_mode.getRandomFinalis();
        this.finalis = path.getFrom();
        this.dominant = path.getTo();
        this.pitch_route = new LinkedList<>();
//        this.melody = new LinkedList<>();
        this.add(new PitchMove(false, List.of(), new PitchPath(null, finalis, 0.0), MusicThought.NULL));
    }
